...
var _ = Describe("VolumeProvider", func() {
	var (
		...
		logger             lager.Logger
		stubCommandFactory gardener.CommandFactory
	)

	BeforeEach(func() {
		cmdRunner = new(fake_command_runner.FakeCommandRunner)
		stubCommandFactory = func(rootfsPath string, uid, gid int, mode os.FileMode, recreate bool, paths ...string) *exec.Cmd {
		...
		volumeProvider = gardener.NewVolumeProvider(volumeCreator, nil, stubCommandFactory, cmdRunner, 5, 5)
